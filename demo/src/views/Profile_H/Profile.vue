<template>
  <div class="mind">
    <div class="mind-body">
      <div class="orange-h"></div>
      <div class="name-h">
        <div class="name-h-top" @click="modifyHJ">
          <div class="name-img-h">
            <img :src="userInfo.avatar" alt="" multiple />
          </div>
          <img src="" class="elf">
          <p>
            <span v-if="token != ''">{{ userInfo.nickname }}</span>
            <span v-else @click.stop="login">登录/注册</span>
          </p>
          <van-icon class="mod-h" name="edit" />
          <span class="name-set-h"><van-icon name="setting-o" /></span>
        </div>
        <ul class="name-h-down">
          <li @click="MyclassHJ">
            <p class="name-h-down-orange">{{leng.length}}</p>
            <p>我的课程</p>
            <p class="name-h-down-gray">-已购课程学习-</p>
          </li>
          <li>
            <p class="name-h-down-orange">0</p>
            <p>我的预约</p>
            <p class="name-h-down-gray">-一对一老师预约-</p>
          </li>
          <li @click="studyCurrencyHJ">
            <p class="name-h-down-orange">0</p>
            <p>剩余学习币</p>
            <p class="name-h-down-gray">-查看剩余学习币-</p>
          </li>
        </ul>
      </div>
      <div class="invitation-h">
        <img
          class="invitation-img-h"
          src="data:image/png;base64,"
          alt=""
        />
        <div class="invitation-txt-h">
          <p>邀请好友注册APP，享多重好礼</p>
          <p>限时特惠，多邀多得</p>
        </div>
        <p class="invitation-arrow-h"><van-icon name="arrow" /></p>
      </div>
      <div class="curriculum-h">
        <div class="title-h">
          <h5>课程相关</h5>
          <ul>
            <li @click="teacherHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">关注的老师</p>
            </li>
            <li @click="collectionHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">我的收藏</p>
            </li>
          </ul>
        </div>
        <div class="title-h">
          <h5>订单相关</h5>
          <ul>
            <li
              v-for="(item, index) in course"
              :key="index"
              @click="courseHJ(item.name)"
            >
              <img :src="item.img" alt="" />
              <p style="color: #b2b2b2">{{ item.name }}</p>
            </li>
          </ul>
        </div>
        <div class="title-h">
          <h5>我的账户</h5>
          <ul>
            <li @click="couponHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">优惠券</p>
            </li>
            <li @click="studyCardHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">学习卡</p>
            </li>
            <li>
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">会员</p>
            </li>
          </ul>
        </div>
        <div class="title-h">
          <h5>自助服务</h5>
          <ul>
            <li @click="newsHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">我的消息</p>
            </li>
            <li>
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">意见反馈</p>
            </li>
            <li>
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">在线服务</p>
            </li>
            <li @click="mysetHJ">
              <img
                src="data:image/png;base64,"
                alt=""
              />
              <p style="color: #b2b2b2">关于我们</p>
            </li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import { userInof,Getcharacteristic } from "@/utils/api";
export default {
  data() {
    return {
      course: [
        {
          img:
            "data:image/png;base64,",
          name: "课程订单",
        },
        {
          img:
            "data:image/png;base64,",
          name: "会员订单",
        },
        {
          img:
            "data:image/png;base64,",
          name: "约课订单",
        },
      ],
      token: this.$store.state.token,
      userInfo: [],
      leng:[],
    };
  },
  created() {
    userInof().then((res) => {
      this.userInfo = res.data;
      console.log(res.data);
    }),
    Getcharacteristic().then((res) => {
      this.leng=res.data.courseList
      console.log(this.leng);
    })
  },
  methods: {
    // 登录
    login() {
      this.$router.push({ path: "/login-h" });
    },
    // 修改个人信息
    modifyHJ() {
      this.$router.push({ path: "/modify-h" });
    },
    // 我的课程
    MyclassHJ() {
      this.$router.push({ path: "/myclass-h" });
    },
    // 我的学习币
    studyCurrencyHJ() {
      this.$router.push({ path: "/study-currency-h" });
    },
    // 关注的老师
    teacherHJ() {
      this.$router.push({ path: "/teacher-h" });
    },
    // 我的收藏
    collectionHJ() {
      this.$router.push({ path: "/collection-h" });
    },
    // 课程订单
    courseHJ(val) {
      this.$router.push({ path: "/course-h", query: { val } });
    },
    // 我的消息
    newsHJ() {
      this.$router.push({ path: "/news-h" });
    },
    // 优惠券
    couponHJ() {
      this.$router.push({ path: "/coupon-h" });
    },
    // 学习卡
    studyCardHJ() {
      this.$router.push({ path: "/study-card-h" });
    },
    // 设置
    mysetHJ() {
      this.$router.push({ path: "/myset-h" });
    },
  },
};
</script>
<style lang="scss" scoped>
.mind {
  width: 100%;
  height: calc(100% - 1rem);
  overflow: scroll;
  position: relative;
  .mind-body {
    width: 100%;
    .orange-h {
      width: 100%;
      height: 2rem;
      border-bottom-left-radius: 20%;
      border-bottom-right-radius: 20%;
      background-color: #fc5500;
    }
    .name-h {
      background-color: white;
      position: relative;
      width: 91.5%;
      margin: 0 auto;
      margin-top: -1rem;
      border-radius: 0.1rem;
      box-shadow: 0 0 3.2vw 0 rgba(0, 0, 0, 0.05);
      .name-h-top {
        width: 100%;
        display: flex;
        font-size: 0.34rem;
        align-items: center;
        padding: 0.3rem;
        .name-img-h {
          width: 1.1rem;
          height: 1.1rem;
          border-radius: 50%;
          margin-right: 0.3rem;
          overflow: hidden;
          // background-color: #ccc;
          position: relative;
          img{
            position: absolute;    
            top: 0;    
            right: 0;    
            bottom: 0;    
            left: 0;    
            margin: auto;    
            width: 1.1rem;
            // transform:scale(1.8);
          }
        }
        .name-set-h {
          position: absolute;
          right: 0.3rem;
          top: 0.3rem;
        }
        .mod-h {
          margin-left: 0.1rem;
          font-size: 0.4rem;
          color: orangered;
        }
      }
      .name-h-down {
        margin-top: 0.3rem;
        display: flex;
        font-size: 0.3rem;
        justify-content: space-around;
        li {
          text-align: center;
          p {
            margin: 0.1rem 0;
          }
          .name-h-down-orange {
            color: orangered;
            font-size: 0.45rem;
          }
          .name-h-down-gray {
            color: rgb(185, 185, 185);
            font-size: 0.1rem;
          }
        }
      }
    }
    .invitation-h {
      width: 91.5%;
      height: 0.8rem;
      background: linear-gradient(90deg, #f2995a, #eb6100);
      margin: 0 auto;
      margin-top: 0.3rem;
      border-radius: 1rem;
      font-size: 0.1rem;
      color: white;
      display: flex;
      align-items: center;
      .invitation-img-h {
        width: 0.5rem;
        margin: 0.25rem;
      }
      .invitation-txt-h {
        p {
          margin-bottom: 0.1rem;
        }
      }
      .invitation-arrow-h {
        position: absolute;
        right: 0.6rem;
      }
    }
    .curriculum-h {
      width: 91.5%;
      overflow: hidden;
      margin: 0 auto;
      .title-h {
        font-size: 0.35rem;
        margin-top: 0.4rem;
        ul {
          padding: 0.4rem 0;
          display: flex;
          text-align: center;
          font-size: 0.2rem;
          border-bottom: #ccc 0.01rem solid;
          li {
            //   margin-right: 0.5rem;
            width: 1.8rem;
            img {
              width: 0.42rem;
              height: 0.42rem;
              margin-bottom: 0.2rem;
            }
          }
        }
      }
    }
  }
}
</style>